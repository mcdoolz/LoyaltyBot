...
IRC.prototype.realtime = function (data){
    ...
    if (__self.streaming) {
        ...
        console.log("Writing to log : "+__self.log);
    }
};
...
IRC.prototype.connect = function () {
    ...
    __self.socket.on('error', function(){
        console.log("Failed to connect.  Your mom.");
        __self.reconnect();
    });
};
...
// join channel
IRC.prototype.joinchannel = function (data) {
    var __self = this;

    if (data[3] === ':>') {
        __self.raw('JOIN ' + __self.config.channel);
        __self.msg("/me comes bearing gifts.");
    }
};
...
IRC.prototype.get_viewers = function (channel) {
  var __self = this;

  console.log("Getting viewers");
  http.get("http://tmi.twitch.tv/group/user/"+channel+"/chatters", function (response) {
  var body = '';
  // put together response
  response.on('data', function (chunk) {
    body += chunk;
  ...
  response.on('end', function () {
      ...
      __self.emit('data', 'DATA - Loaded viewers - Returned: ' + entries);
      __self.viewers = [];
